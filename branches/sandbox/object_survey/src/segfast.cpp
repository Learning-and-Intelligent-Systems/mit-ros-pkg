...
int main(int argc, char **argv) {
    ...
    PtMap pmap(cloud,cluster_tol),pmap2(cloud,cluster_tol),pmap3(cloud,cluster_tol),pmap4(cloud,cluster_tol);

    pmap.randseed=0;
    if(argc==4)
       pmap.randseed=atoi(argv[3]);
...
//    miniHist(10,bounds,"heads have ","heads to search",tosearchsize);
    ...
    cout<<"cloud size: "<<cloud.points.size()<<" heads: "<<pmap4.heads.size()<<" clusters: "<<pmap4.clusters.size()<<std::endl;
