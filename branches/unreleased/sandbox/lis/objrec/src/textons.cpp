...
///////////////////////////////////////////////////////////////////////////////
/*
 * the following is from filters.cpp
 *  by: Bryan Catanzaro
 *  from: Damascene contour detector
 */
...
void textons::initialize(const std::vector<float>& scales_param) {
	if(scales_param.size()==0) {
		this->scales.resize(2);
		this->scales[0] = 2.0f;
		this->scales[1] = 2.0f*M_SQRT2;
	} else {
		scales = scales_param;
	}
	filters.clear();
    float* filters_buffer = 0;
	int n_filter_coefficients;
	int* radii;
	float* scales_array = new float[scales.size()];
	std::copy(scales.begin(),scales.end(),scales_array);
	int filter_count = 17*scales.size();
	createTextonFilters(&filters_buffer, &n_filter_coefficients, &radii, scales_array, scales.size(), elongation);

	int filters_index = 0;
	for(int j=0;j<filter_count;j++) {
		int filter_length = radii[j]*2+1;
		cv::Mat_<float> f(filter_length,filter_length);
		for(int y=0;y<filter_length;y++) {
			for(int x=0;x<filter_length;x++) {
				f(y,x) = filters_buffer[filters_index];
				filters_index++;
			}
		}
		filters.push_back(f);
...
//			cv::Mat_<float>& f = filters[j];
//			cv::Mat_<float> bigger;
//			cv::resize(f,bigger,cv::Size(),5,5,cv::INTER_NEAREST);
//			cv::imshow("filter (enlarged by 5)",objrec::mat_bgr(bigger));
//
//			cv::imshow("filtered image",objrec::mat_bgr(filter_responses[j]));
//
//			cv::waitKey(0);
//		}
}
...
textons::textons(float elongation, const std::vector<float>& scales) : elongation(elongation) {
	initialize(scales);
}
textons::textons(const std::vector<std::vector<float> >& cluster_centers, float elongation,
		const std::vector<float>& scales) : elongation(elongation), cluster_centers(cluster_centers) {
	unsigned int k_clusters = cluster_centers.size();
	if(k_clusters>255) {
		std::ostringstream err;
		err << "maximum number of clusters 255 exceeded (k=" << k_clusters << ")" << std::endl;
		throw std::runtime_error(err.str());
	}
	initialize(scales);
}
textons::textons(const textons& other, const std::vector<std::vector<float> >& cluster_centers) :
	elongation(other.elongation), scales(other.scales), cluster_centers(cluster_centers) {
	filters.resize(other.filters.size());
	for(unsigned int j=0;j<other.filters.size();j++) {
		filters[j] = other.filters[j];
	}
}

void textons::filter_responses(const cv::Mat_<cv::Vec3b>& color_im,
			//output parameter:
			std::vector<cv::Mat_<float> >& responses) const {
	cv::Mat_<unsigned char> gray_im;
	cv::cvtColor(color_im,gray_im,CV_BGR2GRAY);
	cv::Mat_<float> im_float = gray_im;
	responses.clear();
	responses.resize(filters.size());
	for(int j=0;j<(int)filters.size();j++) {
		cv::filter2D(im_float,responses[j],-1,filters[j]);
	}
}

void textons::texton_map(const std::vector<cv::Mat_<float> >& responses,
		//output parameter:
		cv::Mat_<unsigned char>& closest_cluster_indices) const {
	if(responses.size()==0) return;
	closest_cluster_indices.create(cv::Size(responses[0].cols,responses[0].rows));
	for(int y=0;y<responses[0].rows;y++) {
		for(int x=0;x<responses[0].cols;x++) {
			float min_dist2 = std::numeric_limits<float>::infinity();
			int best_cluster = -1;
			for(int k=0;k<(int)cluster_centers.size();k++) {
				float dist2 = 0;
				for(int r=0;r<(int)responses.size();r++) {
					float diff = cluster_centers[k][r]-responses[r](y,x);
					dist2 += diff*diff;
				}
				if(dist2<min_dist2) {
					min_dist2 = dist2;
			...
			closest_cluster_indices(y,x) = best_cluster;
		}
	}
}

std::ostream& operator<<(std::ostream& out, const textons& t) {
	out << "#<filter elongation>" << std::endl;
	out << t.elongation << std::endl;
	out << "#<number of scales>" << std::endl;;
	unsigned int n_scales = t.scales.size();
	out << n_scales << std::endl;
	out << "# <scale value>" << std::endl;
	for(unsigned int j=0;j<n_scales;j++) {
		out << " " << t.scales[j] << std::endl;
	}
	unsigned int k_clusters = t.cluster_centers.size();
	out << "#<number of clusters>" << std::endl;
	out << k_clusters << std::endl;
	int n_filters = 13*n_scales;
	out << "# <mean value for filter 1> ... <mean value for filter " << n_filters << ">" << std::endl;
	for(unsigned int k=0;k<k_clusters;k++) {
		for(int j=0;j<n_filters;j++) {
			out << " " << t.cluster_centers[k][j];
		}
		out << std::endl;
	}
	return out;
...
textons::textons(std::istream& in, int& line_number) {
	in.exceptions(std::ios::failbit);
	std::istringstream elongation_line(nextline(in,line_number));
	try {
		elongation_line >> elongation;
	} catch(...) {
		std::ostringstream err;
		err << "Error reading elongation value for textons on line " << line_number << std::endl;
		throw std::runtime_error(err.str());
	}

	unsigned int n_scales;
	std::istringstream n_scales_line(nextline(in,line_number));
	try {
		n_scales_line >> n_scales;
	} catch(...) {
		std::ostringstream err;
		err << "Error reading number of scales for textons on line " << line_number << std::endl;
		throw std::runtime_error(err.str());
	}
	...
	try {
		for(unsigned int j=0;j<n_scales;j++) {
			std::istringstream scales_line(nextline(in,line_number));
			scales_line >> scales[j];
		}
	} catch(...) {
		std::ostringstream err;
		err << "Error reading scales for textons on line " << line_number << std::endl;
		throw std::runtime_error(err.str());
	}

	unsigned int k_clusters;
	std::istringstream k_clusters_line(nextline(in,line_number));
	try {
		k_clusters_line >> k_clusters;
	} catch(...) {
		std::ostringstream err;
		err << "Error reading number of clusters for textons on line " << line_number << std::endl;
		throw std::runtime_error(err.str());
	}
	if(k_clusters>254) {
		std::ostringstream err;
		err << "maximum number of clusters 254 exceeded (k=" << k_clusters << ") on line " << line_number << std::endl;
		throw std::runtime_error(err.str());
	}

	int n_filters = 13*n_scales;
	cluster_centers.resize(k_clusters);
	for(unsigned int k=0;k<k_clusters;k++) {
		std::istringstream cluster_line(nextline(in,line_number));
		try {
			cluster_centers[k].resize(n_filters);
			for(int j=0;j<n_filters;j++) {
				cluster_line >> cluster_centers[k][j];
			}
		} catch(...) {
			std::ostringstream err;
			err << "Error reading cluster number " << k << " for textons on line " << line_number << std::endl;
			throw std::runtime_error(err.str());
		}
	}
	initialize(scales);
...
cv::Mat_<cv::Vec3b> textons_bgr(const cv::Mat_<cv::Vec3b>& picture, const textons& t) {
	cv::Mat_<cv::Vec3b> textonsbgr(picture.rows,picture.cols);
	cv::Mat_<unsigned char> texton_map = t.texton_map(picture);
	textonsbgr.create(texton_map.size());
	if(t.n_clusters()==0) {
		set_to(textonsbgr,cv::Vec3b(0,0,0));
		return textonsbgr;
	}
	unsigned char gray_width = (unsigned int)256/t.n_clusters();
	for(int y=0;y<texton_map.rows;y++) {
		for(int x=0;x<texton_map.cols;x++) {
			unsigned char v = texton_map(y,x)*gray_width;
			textonsbgr(y,x) = cv::Vec3b(v,v,v);
		}
	}
	return textonsbgr;
}
