...
void getSkeletonJoint(XnUserID player, kinect_tools::SkeletonJoint &j, XnSkeletonJoint name){
...
void getSkeleton(XnUserID player,kinect_tools::Skeleton &skel){
...
void getSkels(std::vector<mapping_msgs::PolygonalMap> &pmaps, kinect_tools::Skeletons &skels){
   XnUserID aUsers[15];
   XnUInt16 nUsers = 15;
...
//   param_nh.param ("device_id", device_id, argc > 1 ? atoi (argv[1]) : 0);
   ...
   skel_pub = nh_.advertise<kinect_tools::Skeletons> ("skeletons", 100);
