...
TABLE_MIDDLE_X = 0.35 #keep this at least 0.3 so it can't see its hand
TABLE_HEIGHT = 0.7
TABLE_INCR = 0.2
TABLE_START_Y = 0.45
TABLE_TOL = 0.15
...
class HeadController:
    def __init__(self, manager=None):
        self.left_arm_control = pr2_utils.arm_control.ArmControl\
            ('left_arm', manager=manager)
        self.right_arm_control = pr2_utils.arm_control.ArmControl\
            ('right_arm', manager=self.left_arm_control.manager)
        self.manager = self.left_arm_control.manager

    def lookAt(self, point_stamped):
        self.left_arm_control.move_arm_to_side()
        self.right_arm_control.move_arm_to_side()
        r = self.manager.point_head\
            (pr2_utils.conversions.pointToList(point_stamped.point),\
    ...
    def findTableEdge(self, side):
        if side == 'l':
            return self.findTableLeftEdge()
        else:
            rospy.logerr('Unable to find table right edge: not implemented')
            return None
