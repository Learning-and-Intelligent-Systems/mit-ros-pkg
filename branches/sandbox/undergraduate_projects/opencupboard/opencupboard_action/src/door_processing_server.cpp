		       ...
		       pcl::PointCloud<pcl::PointXYZ> &object_cloud)
{
  ...
  for (int i=0;i<int(cloud.points.size());i+=2)
    {
  ...
  ROS_INFO("in func object_cloud size=%d", int(object_cloud.points.size()));
}
...
bool add(opencupboard_msgs::cabinet_table::Request  &req,
         opencupboard_msgs::cabinet_table::Response &res )
{
  // Get pointcloud
  ros::NodeHandle nh;
  ...
  ROS_INFO("orig cloud size=%d",int(cloud.points.size()));
  ...
  ROS_INFO("object_cloud size=%d", int(object_cloud.size()));
