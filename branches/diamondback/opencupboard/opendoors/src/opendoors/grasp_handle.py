...
class GraspHandleAction():
	def __init__(self, name):
		...
		'''
		instantiates an SimpleActionServer with and starts it immediately. The action server is 
		named with the given string. It is specified to run the main1() function when actionStarts
		'''		
		self._as = actionlib.SimpleActionServer(self._action_name, opendoors.msg.GraspHandleAction, execute_cb=self.main1)
	...
	def interpolatedIK(self,start, end, arm, handle_box):
		'''
		Moves the specified hand from the start pose to the end pose using move_arm_service/srv/InterpolatedIK 			service.
		
		@type start: PoseStamped
		@param start: PoseStamped start is used as start_pose field for InterpolatedIKRequest
		...
		'''
		ops = motion_planning_msgs.msg.OrderedCollisionOperations()
	...
	def main1 (self, goal ):
		'''
		This function is called with the goal msg whenever request to GraspHandleAction is made. It moves the specified arm in the goal to a position where its gripper is 0.1 (10 cm) in front of the specified pose, meaning its 			x position is 0.1 less. It sends MoveArmRequest to a MoveArm service to move the arm.Note that MoveArm service moves that wrist of the arm to the specified position. Based on the specified arm it initializes 		SimpleActionClient (either /l_gripper_controller/gripper_action or /r_gripper_controller/gripper_action) and uses it to open and close the gripper. Using InterPolatedIK service, it then moves the arm straight in x axis 			by 0.1 to finally reach the goal pose and closes the gripper.
	...
	def moveArm(self, pose, arm):
		'''
		Moves the specified arm to the specified pose using MoveArm service which uses IK. Initializes MoveArmService and a MoveArmRequest to the service with specified arm and pose.
		...
		'''
		# rospy.loginfo("waiting for move_arm_to_pose to come up")
		# rospy.wait_for_service("move_arm_to_pose")
#		move_call = move_arm_service.srv.MoveArmRequest()
		# move_call = arm_control.move_arm_collision_free()
		# move_call.pose_stamped = pose
		# move_call.arm = arm
#		print "Specified arm:", arm
#		move_srv = rospy.ServiceProxy('move_arm_to_pose',move_arm_service.srv.MoveArm)
#		move_srv = rospy.ServiceProxy('move_arm_to_pose', arm_control.move_arm_collision_free)
#		print "Initializing the server for ",arm
		print 'Moving arm'
