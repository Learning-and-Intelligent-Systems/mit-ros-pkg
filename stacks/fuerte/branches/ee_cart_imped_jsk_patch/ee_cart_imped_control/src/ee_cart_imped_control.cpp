...
(const _Matrix_Type_ &a, _Matrix_Type_ &result,
 ...
 std::numeric_limits<typename _Matrix_Type_::Scalar>::epsilon()) {
  ...
  typename _Matrix_Type_::Scalar tolerance = epsilon * 
    std::max(a.cols(),
...
EECartImpedControlClass::sampleInterpolation() {
  ...
  if (current_goal_index > 0 && last_point_.time_from_start.toSec() != 
    ...
    last_point_.forceDirection = 
      desiredPoses[current_goal_index-1].forceDirection;
  ...
  if (segEndTime <= segStartTime) {
    ...
    qs.setX(start_point.forceDirection.x);
    qs.setY(start_point.forceDirection.y);
    qs.setZ(start_point.forceDirection.z);
    qs.setW(start_point.forceDirection.w);

    qe.setX(end_point.forceDirection.x);
    qe.setY(end_point.forceDirection.y);
    qe.setZ(end_point.forceDirection.z);
    qe.setW(end_point.forceDirection.w);
    qq = tf::slerp(qs, qe, r);
