...
void EECartImpedExecuter::goalCB(GoalHandle gh) {
  ...
  ee_cart_imped_control::EECartImpedGoal transformed_traj;
  ...
  for (size_t i = 0; i < current_traj_.trajectory.size(); i++) {
    ee_cart_imped_control::StiffPoint &currpt = current_traj_.trajectory[i];
	...
	!currpt.header.frame_id.empty()) {
      ...
      ee_cart_imped_control::StiffPoint transpt;
