...
#include "tabletop_for_cabinet/cabinet_table_result.h"
 ...
 */
bool add(tabletop_for_cabinet::cabinet_table_result::Request &req,
         tabletop_for_cabinet::cabinet_table_result::Response &res )
{
  ros::NodeHandle nh;
...
int main(int argc, char **argv)
{
  ...
  return 0;
      ...
      // for (int j=0;j<req.detection.clusters[i].points.size();j++)
      // 	{
      // 	  avg.x = req.detection.clusters[i].points[j].x + avg.x;
      // 	  avg.y = req.detection.clusters[i].points[j].y + avg.y;
      // 	  avg.z = req.detection.clusters[i].points[j].z + avg.z;
      // 	}
      // avg.x = avg.x/req.detection.clusters[i].points.size();
      // avg.y = avg.y/req.detection.clusters[i].points.size();
      // avg.z = avg.z/req.detection.clusters[i].points.size();
      // ROS_INFO("avg x=%f y=%f z-%f", avg.x, avg.y, avg.z);
      // make_marker(req.detection.table.pose,visualization_msgs::Marker::SPHERE,
      // 		  avg.x,avg.y,avg.z,0.05,0.05,0.05,1.0,1.0,0.0);


      // // makes markers for point clouds
      // for (int k=0;k<req.detection.clusters.size();k++)
      // 	{
      // 	  visualization_msgs::Marker marker;
      // 	  marker.header.frame_id = req.detection.clusters[i].header.frame_id;
      ...
      // 	  for (size_t j =0; j<req.detection.clusters[i].points.size();j++)
      // 	    {
      // 	      geometry_msgs::Point p;
      // 	      p.x = req.detection.clusters[i].points[j].x;
      // 	      p.y = req.detection.clusters[i].points[j].y;
      // 	      p.z = req.detection.clusters[i].points[j].z;
	  ...
	  // geometry_msgs::Point32 box_middle, new_box_middle;
	  // box_middle.x = (new_max.x+new_min.x)/2;
	  // box_middle.y = (new_max.y+new_min.y)/2;
	  // box_middle.z = (new_max.z+new_min.z)/2;
	  // inverse_transform_point(req.detection.table.pose.pose, box_middle, new_box_middle);
	  // make_marker(req.detection.table.pose, visualization_msgs::Marker::SPHERE,
	  // 	      new_box_middle.x,new_box_middle.y,new_box_middle.z,
