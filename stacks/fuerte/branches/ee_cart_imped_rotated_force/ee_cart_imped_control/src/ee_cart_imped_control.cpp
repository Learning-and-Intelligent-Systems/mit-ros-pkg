...
(const ee_cart_imped_msgs::EECartImpedGoalConstPtr &msg) {
  ...
  try{
    tflistener.lookupTransform("/base_link",frame_id,ros::Time(0),tftransform);
    tf::Quaternion quaternion = tftransform.getRotation();
    new_traj.force_rotation = KDL::Rotation::Quaternion(quaternion.getX(),quaternion.getY(),quaternion.getZ(),quaternion.getW());
  }
  catch (tf::TransformException ex){
    new_traj.force_rotation = KDL::Rotation();
...
void EECartImpedControlClass::update()
{
    ...
    boost::shared_ptr<const EECartImpedData> desired_poses_ptr;
    desired_poses_box_.get(desired_poses_ptr);
    forceRotation = desired_poses_ptr->force_rotation;
