...
cardboard::ModelManager *model_manager;
...
static double adjust_table_height(const PointCloudXYZ &cloud)
{
  cardboard::init_rand();

  int i, j, iter = 50, n = cloud.points.size();
  double thresh = .01;

  double zmin = 0, dmin = 1000*n;
  for (i = 0; i < iter; i++) {

    double z = cloud.points[rand() % n].z;
...
void point_cloud_callback2(sensor_msgs::PointCloud2 msg);
...
void point_cloud_callback2(sensor_msgs::PointCloud2 msg)
{
  //ROS_INFO("Got PointCloud msg with %u points\n", msg.width * msg.height);

  // get the cloud in PCL format in the right coordinate frame
  PointCloudXYZ cloud, cloud2, full_cloud;
  pcl::fromROSMsg(msg, cloud);
  //pcl_ros::transformPointCloud(target_frame, cloud, cloud2, *tf_listener);
  ...
  cardboard::SceneHypothesis detected_objects;
  ...
  //for (size_t i = 1; i < 3; i++) {  //dbug
    ...
    //cardboard::visualize_range_image_with_normals(range_image, range_image_normals);  //dbug
    ...
    cardboard::ObjectHypothesis obj = cardboard::fit_object(*model_manager, &model_tester, init_pose, table_height);


    //cardboard::PointCloudNormalTester
    ...
    cardboard::range_image_with_normals_to_points_and_normals(range_image, range_image_normals, obs_cloud, obs_normals);
    ...
    pcl::transformPointCloudWithNormals(model_manager->get_model(obj.name).cloud_normals, model_cloud_normals,
					cardboard::pose_to_affine_matrix(obj.pose));
    PointCloudXYZ model_cloud;
    PointCloudN model_normals;
    cardboard::point_cloud_normals_to_points_and_normals(model_cloud_normals, model_cloud, model_normals);
...
void table_polygons_callback(furniture::Table_Polygons msg)
{
  ...
  if (msg.polygons.size() > 0) {
    // hack: assume first table is the one we want
    table_polygon.points.resize(msg.polygons[0].points.size());
    for (size_t i = 0; i < msg.polygons[0].points.size(); i++) {
      table_polygon.points[i].x = msg.polygons[0].points[i].x;
      table_polygon.points[i].y = msg.polygons[0].points[i].y;
      table_polygon.points[i].z = msg.polygons[0].points[i].z;
    }
    have_table = true;
  }
}
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber sub_cloud = nh.subscribe("pointcloud", 1, point_cloud_callback);
  ros::Subscriber sub_cloud2 = nh.subscribe("pointcloud2", 1, point_cloud_callback2);
  ...
  if (nh.getParam("/object_detector/objects_folder", objects_folder))
    ROS_INFO("Got param: %s", objects_folder.c_str());
  else
    ROS_ERROR("Failed to get param 'objects_folder'");

  //nh.param<string>("cardboard/objects_folder", objects_folder, "data");
  printf("%s\n", objects_folder.c_str());
  model_manager = new cardboard::ModelManager(objects_folder);

  nh.param("/object_detector/table_filter_thresh", table_filter_thresh, .02);
