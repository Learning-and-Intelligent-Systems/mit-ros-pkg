...
namespace ee_cart_imped_control_ns {
  ...
  private:
    ...
    /// The chain of links and joints in PR2 language for reference
    /// and commanding in update loop
    pr2_mechanism_model::Chain chain_;
    /// The chain of links and joints in PR2 language for reference
    /// only in the command callback
    //Read-only after initialization
    ...
    ///The time at which the goal we saw
    ///on the last interation was started
    //Referenced only in update loop
    double last_goal_starting_time_;
    
    typedef std::vector<ee_cart_imped_control::StiffPoint> EECartImpedTraj;
    ...
    ///The actual position of the robot when we
    ///achieved the last point on the trajectory
    //Referenced only in updated loop
    ee_cart_imped_control::StiffPoint last_point_;
    ...
    */
     ...
     *
     * @return A <A HREF=http://www.ros.org/doc/api/ee_cart_imped_control/html/msg/StiffPoint.html>StiffPoint</A> that is the point
