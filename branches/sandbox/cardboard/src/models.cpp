...
ModelManager::ModelManager(string object_folder_name)
{
  object_folder = object_folder_name;
...
void ModelManager::load_models()
{
  ...
  sprintf(fname, "%s/%s", object_folder.c_str(),
	  object_names_file_const.c_str());
  ...
  for (int i = 0; i < num_objects; i++) {
    ...
    // load PCD file
    sprintf(fname, "%s/%s.pcd", object_folder.c_str(),
	    models[i].name.c_str());
    ...
    //		sprintf(fname, "%s/%sHull.ply", object_folder.c_str(),
    //				models[i].name.c_str());
    //
    ...
    */
    ...
    // convert models from cm to m
    //		Vector4f centroid;
    //		pcl::compute3DCentroid(cloud, centroid);
    //		for (size_t j = 0; j < cloud.points.size(); j++) {
    //			cloud.points[j].x = (cloud.points[j].x) / 100.;
    //			cloud.points[j].y = (cloud.points[j].y) / 100.;
    //			cloud.points[j].z = (cloud.points[j].z) / 100.;
    //		}
    //		for (int j = 0; j < num_vertices; j++) {
    //			hull_vertices[j].x = (hull_vertices[j].x) / 100.;
    //			hull_vertices[j].y = (hull_vertices[j].y) / 100.;
    //			hull_vertices[j].z = (hull_vertices[j].z) / 100.;
    //		}
    ...
    //			Vector3f p1 = vertex_to_eigen(
    //					&hull_vertices[hull_faces[j].verts[0]]);
    //			Vector3f p2 = vertex_to_eigen(
    //					&hull_vertices[hull_faces[j].verts[1]]);
    //			Vector3f p3 = vertex_to_eigen(
    //					&hull_vertices[hull_faces[j].verts[2]]);
...
/* add a new model */
void ModelManager::add_model(string model_name, pcl::PointCloud<pcl::PointXYZ> &cloud)
{
  int i = models.size();
  models.resize(i + 1);
  models[i].name = model_name;
  models[i].cloud = cloud;

  // create distance transform
  float resolution = .005;
  float padding = .05;
  models[i].distance_transform.init(cloud, resolution, padding);
}


/* get model at pose (affine) */
bool ModelManager::get_model_at_pose(string model_name, Matrix4f pose_affine,
...
/* get model at pose */
bool ModelManager::get_model_at_pose(string model_name,
				     geometry_msgs::Pose pose, pcl::PointCloud<pcl::PointXYZ> &cloud) const
{
